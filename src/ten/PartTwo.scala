...
object PartTwo {
  def main(args: Array[String]) {
    ...
    val boundingBox = getBoundingBox(closePoints._1)
    printPoints(closePoints._1, boundingBox)
    println(closePoints._2)
  }
  ...
  def getClosePoints(points: List[Point]): (List[Point], Int) = {
    @tailrec
    def helper(points: List[Point], acc :Int): (List[Point], Int) = {
      val newPoints = doStep(points)
      if (arePointsClose(newPoints))
        (newPoints, acc)
      else
        helper(newPoints, acc + 1)
    }
    helper(points, 1)
  }
}
