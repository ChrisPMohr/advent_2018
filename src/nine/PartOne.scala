...
object PartOne {
  def main(args: Array[String]) {
    ...
    val scores = mutable.HashMap.empty[Int, Int]
  ...
  case class NonEmpty(elem: Int, var next: Node, var prev: Node) extends Node {
    def print(): Unit = {
      Predef.print(s"($elem)")
    }
  }
  ...
  def insertMarble(marbles: DoublyLinkedList, scores: mutable.HashMap[Int, Int], marble: Int, player: Int): Unit = {
    if (marble > 0 && marble % 23 == 0) {
      val oldScore = scores.getOrElse(player, 0)
